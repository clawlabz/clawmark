...
| # | Feature | Description |
|---|---------|-------------|
| 1 | Dark/Light Theme Toggle | System-aware with manual override |
| 2 | Bulk Delete | Select multiple bookmarks and delete |
| 3 | Sort Options | Sort by date added, title, most recently accessed |
| 4 | Favicon Display | Show site favicon next to bookmark |

### P2 — Nice to Have (Future)

| # | Feature | Description |
|---|---------|-------------|
| 1 | Browser Extension | One-click save from any page |
...
- Given I enter a duplicate URL, When I save, Then I see a warning but can still save
...
**Acceptance Criteria:**
- Given I click "Import" and select a valid JSON file, When parsing succeeds, Then bookmarks are merged (duplicates by URL are skipped)
- Given I upload an invalid file, When parsing fails, Then I see an error message and no data is changed
...
- Cross-device sync
- Browser extension
- Auto-fetching page titles from URLs
...
- Removed "auto-fetch title" from MVP — adds network dependency, contradicts pure client-side goal
- Added duplicate URL warning to US-001 — common edge case
- Added empty state to US-002 — first-time user experience matters
- Moved dark/light theme to P1 — not essential for MVP, adds complexity
- Clarified search is AND logic when combined with tag filter

### Round 2: User Perspective Review
- Confirmed: core flow (add → search → find) is achievable in < 10 seconds
- Added Escape key to dismiss edit dialog (US-004) — expected behavior
- Verified import shows summary of what happened — user needs feedback
- Confirmed no unnecessary features — every P0 item directly serves bookmark management
- Noted: keyboard shortcut (Cmd/Ctrl+K) is critical for power users, kept as P0
